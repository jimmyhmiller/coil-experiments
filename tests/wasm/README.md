...
scripts/wasm-spec.sh test-all
```

`test-all` is the release gate: it runs every harness below, covering all 73
pinned MVP core WAST files, followed by the direct Coil-native WAT reader checks.
Run it under the repository's 30 GiB aggregate process-tree watchdog for an
unattended conformance audit.

`test-integers` passes all 830 assertions in the official `i32` and `i64` files:
700 returns, 112 validation failures, and 18 runtime traps. Mixed-result integer
comparisons are included, and returns run in one generated Coil entry point per
module. This is a semantic test—not merely a module compile check.

`test-floats` executes all 3,178 ordinary `f32`/`f64` return assertions, 1,822
canonical- and arithmetic-NaN assertions, and 22 validation failures. Floating inputs and results are
...
`test-conversions` executes 334 mixed-scalar conversion results, eight mixed-width
NaN results, 25 validation failures, and 67 overflow/invalid-conversion traps as
actual process traps. Compiler diagnostics do not count as passing trap assertions.
...
assertions, and all 38 official `memory_size` assertions across four modules.

`test-tables` checks focused table initialization and indirect dispatch, then
passes every assertion in the official MVP `call_indirect` file: 103 returns, 13
process traps, 22 validation failures, 11 malformed modules, and two exhaustion
assertions.
...
12 validation failures, and two malformed modules. This includes loops in operand positions, deep
...
currently execute 6,706 official assertions.
...
each target as a lexical Coil transfer. The cumulative official total is 7,393
assertions.
...
official assertion total is 7,403.
...
`unreachable` files: 357 returns, 129 validation failures, 64 runtime traps, and
two exhaustion assertions. The cumulative official assertion total is 7,955.
...
stack values. The cumulative official assertion total is 8,078.
...
returns, 38 validation failures, 16 malformed-text rejections, 14 runtime
traps, and one exhaustion assertion. This covers recursion, forward calls,
function/table index validation, deep stack use, and trap unwinding. The
cumulative official assertion total is 8,292.
...
validation are covered. The cumulative official assertion total is 8,365.
...
cumulative official assertion total is 9,225.
...
type-declaration module. The cumulative official assertion total is 9,229.
...
official assertion total is 9,249.
...
official assertion total is 9,389.
...
The cumulative official assertion total is 9,483.
...
UTF-8 validation. The cumulative official assertion total is 10,318.
...
external kind. The cumulative official assertion total is 10,346.
...
integer comparison results. The cumulative official assertion total is 15,884.
...
official assertion total is 15,973.
...
cumulative official assertion total is 16,353.
...
The cumulative official assertion total is 16,832.
...
official assertion total is 17,304.
...
all 96 module commands. The cumulative official assertion total is 18,098.
...
The cumulative official assertion total is 18,257.
...
cumulative official assertion total is 18,317.
...
The cumulative official assertion total is 18,428.
