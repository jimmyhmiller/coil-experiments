...
scratch_dir=$(mktemp -d "${TMPDIR:-/tmp}/react-hir-parse.XXXXXX")
...
"$coil_compiler" build "$benchmark_dir/react-hir-parse.coil" --release \
  --link-flag "$scratch_dir/benchmark_barrier.o" \
  -o "$scratch_dir/react-hir-parse-benchmark"

cd "$project_dir"
"$scratch_dir/react-hir-parse-benchmark"
