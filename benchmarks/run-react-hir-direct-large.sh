...
scratch_dir=$(mktemp -d "${TMPDIR:-/tmp}/react-hir-direct-large.XXXXXX")
...
"$coil_compiler" build "$benchmark_dir/react-hir-direct-large.coil" --release \
  --link-flag "$scratch_dir/benchmark_barrier.o" \
  -o "$scratch_dir/react-hir-direct-large"

cd "$project_dir"
"$scratch_dir/react-hir-direct-large" "$@"
