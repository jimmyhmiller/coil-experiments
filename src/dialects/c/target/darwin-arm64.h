/* Predefined macros for Darwin on arm64.
 *
 * A C compiler has to describe its target to the preprocessor before the first
 * line of a program is read: the system headers decide which declarations exist
 * by testing these. Keeping them here as ordinary #defines, force-included
 * ahead of the translation unit, means the set stays readable and editable
 * rather than buried in the driver.
 *
 * The values were taken from the system compiler's own set (clang -dM -E) so
 * that the headers see exactly the target they expect. Nothing here names this
 * compiler as clang, and macros that would steer a header into a dialect this
 * compiler does not implement are deliberately absent -- __BLOCKS__ above all,
 * which would turn on the ^-block syntax.
 */
...
#define __VERSION__ "Homebrew Clang 22.1.8"
