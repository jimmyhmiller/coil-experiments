...
(defstruct Particle
  ...
  },
  {
    ...
    watch:'Apply, then press Run. The same particles continue from the frozen positions, now carrying Visibility values.',
    source:`(defsum Visibility (Hidden) (Visible))
...
(defn kind-of [(p Particle)] (-> Kind)
  ...
  },
  {
    ...
    watch:'Apply, then Run. Bars appear on the far right and the same world continues.',
...
async function submit(src,reason='apply'){if(!src.trim())return;if(editing){queued={src,reason};return}editing=true;diagnostic.className='diagnostic';diagnostic.textContent=`${reason}: checking and compiling the snippet…`;try{let r=await fetch('/api/live/edit',{method:'POST',headers:{'Content-Type':'text/plain'},body:src});if(!r.ok&&r.status!==202)throw Error(`HTTP ${r.status}`);let s=await terminal();applied.add(step);if(s.result!==0)await fetch('/api/native-demo/pause',{method:'POST'})}catch(e){diagnostic.className='diagnostic bad';diagnostic.textContent=e.message}finally{editing=false;if(queued){let q=queued;queued=null;submit(q.src,q.reason)}}}
editor.oninput=()=>{stepSources[step]=editor.value;renderHighlight();if($('live-typing').checked){clearTimeout(liveTimer);liveTimer=setTimeout(()=>{let item=itemAt(editor.value,editor.selectionStart);if(balanced(item))submit(item,'live edit')},16)}};editor.onscroll=renderHighlight;$('apply').onclick=()=>submit(editor.value);$('back').onclick=()=>{if(step>0){step--;renderStep()}};$('next').onclick=()=>{if(step<STEPS.length-1&&applied.has(step)){step++;renderStep()}};
...
renderStep();setInterval(async()=>{try{showStatus(await apiStatus())}catch{}},700);apiStatus().then(showStatus).catch(()=>{});
